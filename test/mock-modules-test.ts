...
const url = `${domainUrl}/page?mock=mock-modules`;

fixture('Mock module tests').page(url);
...
test
    ...
    ('validate renderPage div render', async (testController: TestController) => {
        ...
        await testController.expect(renderPageDiv.exists).eql(true, 'Could not find renderPage div');
    });